...
export function SpaceAdministration({ channel, roster, registrarRoster, state, rootState, version = 0, daemons, channelDevices, disabled, onSubmit, onRefresh, onClose }) {
  ...
  return <SidePanel className="space-administration" ariaLabel="空间管理" eyebrow="SPACE CONTROL" title="空间管理" tabs={TABS} activeTab={tab} onTabChange={setTab} onClose={onClose}>
    ...
    <div hidden={tab !== 'channels'}><ChannelTemplatesPanel states={states} registrarRoster={registrarRoster} disabled={disabled} onSubmit={onSubmit} /></div>
		<div hidden={tab !== 'config'}><ChannelConfigurationPanel channel={channel} devices={channelDevices} roster={roster} states={states} version={version} disabled={disabled} onSubmit={onSubmit} onRefresh={onRefresh} /></div>
    <div hidden={tab !== 'devices'}><DevicesPanel channel={channel} states={states} version={version} daemons={daemons} channelDevices={channelDevices} registrarRoster={registrarRoster} disabled={disabled} onSubmit={onSubmit} onRefresh={onRefresh} /></div>
  </SidePanel>;
}
