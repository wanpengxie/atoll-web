...
export function WorkItemContext({ item, roster = [], onSource, onResolve, onOpenTurn, onRetry, onCancelAutomation, onClose }) {
  ...
  const assignees = item.assigneeActorIds.map((id) => names.get(id) || id).join('、') || '未指定';
  return <SidePanel className={`work-item-context kind-${item.kind}`} ariaLabel="工作项详情" eyebrow={KIND_LABELS[item.kind]} title={item.title} closeLabel="关闭工作项详情" onClose={onClose}>
    ...
    <dl className="work-item-metadata"><dt>类型</dt><dd>{KIND_LABELS[item.kind]}</dd><dt>负责人</dt><dd>{assignees}</dd><dt>创建者</dt><dd>{names.get(item.requesterActorId) || item.requesterActorId || '未知'}</dd><dt>{item.kind === 'approval' ? '到期' : '截止/触发'}</dt><dd>{date(item.dueAt)}</dd><dt>事实来源</dt><dd>{item.provenance === 'ledger' ? '频道账本' : '本设备持久记录'}</dd></dl>
    ...
    {item.kind === 'task' && <section className="work-item-detail"><h3>正式任务</h3><p>编号：{item.nativeId}</p><p>该任务由 {names.get(item.diagnostic?.providerActorId) || item.diagnostic?.providerActorId || '能力提供者'} 返回稳定任务编号；更新动作只会在 provider 声明对应能力时出现。</p></section>}
