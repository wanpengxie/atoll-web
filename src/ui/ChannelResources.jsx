...
export function ChannelResources({ channel, devices = [], disabled, attachDisabled, attachDisabledReason, onFileOperation, onAttach, onClose, surface = 'context' }) {
  const [tab, setTab] = useState('files');
  const content = <>
    <div hidden={tab !== 'kv'}><KeyValuePanel channel={channel} disabled={disabled} onFileOperation={onFileOperation} /></div>
    <div hidden={tab !== 'files'}><FilesPanel channel={channel} devices={devices} disabled={disabled} attachDisabled={attachDisabled} attachDisabledReason={attachDisabledReason} onFileOperation={onFileOperation} onAttach={onAttach} /></div>
  </>;
