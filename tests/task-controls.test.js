...
import { controlLabel, createWaitingTargetAuthority, extraControls, taskControlContext } from '../src/model/task-controls.js';
...
describe('task control eligibility', () => {
  ...
  it('lets any writing member act on work it did not send', () => {
    expect(taskControlContext(processing, { selfId: 'other', access: 'member_active', targetAuthority: currentTarget })).toMatchObject({ canEdit: true, canStop: true });
    expect(taskControlContext(queued, { selfId: 'other', access: 'member_active', targetAuthority: currentTarget })).toMatchObject({ canInsert: true, canEdit: true });
  });
  ...
  it('cancels your own request, and asks the holder to drop anyone else\'s', () => {
    ...
    expect(taskControlContext(queued, { selfId: 'other', access: 'member_active', targetAuthority: currentTarget })).toMatchObject({ canCancel: true, cancelsAsDismiss: true });
    // 不可写的频道两条路都没有。
  ...
  it('keeps caller cancel but disables receiver-directed controls until exact roster authority is current', () => {
    const unknown = taskControlContext(queued, { selfId: 'me', access: 'member_active' });
    expect(unknown).toMatchObject({
      targetCurrentness: 'unknown', callerCancelEligible: true, targetControlsEligible: false,
      canCancel: true, cancelsAsDismiss: false, canInsert: false, canEdit: false,
    });
    const departed = taskControlContext(queued, {
      selfId: 'me', access: 'member_active',
      targetAuthority: { current: true, actorIDs: new Set(['agent:worker:200']) },
    });
    expect(departed).toMatchObject({
      targetCurrentness: 'departed', callerCancelEligible: true, targetControlsEligible: false,
      canCancel: true, canInsert: false, canEdit: false,
    });
    const thirdParty = taskControlContext(queued, { selfId: 'other', access: 'member_active' });
    expect(thirdParty).toMatchObject({
      targetCurrentness: 'unknown', callerCancelEligible: false, targetControlsEligible: false,
      canCancel: false, cancelsAsDismiss: false,
    });

    const oldIncarnation = {
      ...queued,
      request: { ...queued.request, audience: ['agent:worker:100'] },
    };
    expect(taskControlContext(oldIncarnation, {
      selfId: 'me', access: 'member_active',
      targetAuthority: { current: true, actorIDs: new Set(['agent:worker:200']) },
    }).targetCurrentness).toBe('departed');
  });

  it('requires principal, channel, generation, completeness and exact actor IDs for current authority', () => {
    const input = {
      principalId: 'principal-1', channelId: 'c1', generation: 7,
      roster: [
        { id: 'human:principal-1:1', kind: 'human' },
        { id: 'agent:worker:200', kind: 'agent' },
      ],
    };
    const current = createWaitingTargetAuthority({
      ...input,
      rosterAuthority: { principalId: 'principal-1', channelId: 'c1', generation: 7, current: true },
    });
    expect(current.current).toBe(true);
    expect([...current.actorIDs]).toEqual(['agent:worker:200']);
    expect(createWaitingTargetAuthority({
      ...input,
      rosterAuthority: { principalId: 'principal-1', channelId: 'c1', generation: 6, current: true },
    }).current).toBe(false);
    expect(createWaitingTargetAuthority({
      ...input,
      rosterAuthority: { principalId: 'principal-1', channelId: 'c1', generation: 7, current: false },
    }).current).toBe(false);
  });
});
