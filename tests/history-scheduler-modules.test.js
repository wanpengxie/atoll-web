...
function physicalBatch({ source, channelId = 'c0', beforeSeq, limit = 3, byteLimit = 100, generation = 1 }) {
  return {
    authority: { principalEpoch: 0, worldEpoch: 0, generation, attachEpoch: 0 },
    channelId, beforeSeq, limit, byteLimit, source, ref: '',
  };
}
...
describe('history scheduler modules', () => {
  ...
  it('keeps network adaptation to I/O and validation facts', async () => {
    ...
    const batch = physicalBatch({ source: 'network', beforeSeq: 11, limit: 10, byteLimit: 1_000, generation: 7 });
    adapters.prepare(batch);
    const execution = adapters.execute(batch, { priority: 'foreground' });
    await vi.waitFor(() => expect(registered?.ref).toBe('page-1'));
    expect(Object.keys(registered).sort()).toEqual([
      'authority', 'beforeSeq', 'byteLimit', 'channelId', 'limit', 'ref', 'source',
    ]);
  ...
  it('gives cancellation and failure one terminal source status', async () => {
    ...
    const failed = physicalBatch({ source: 'network', beforeSeq: 2, generation: 1 });
    ...
    const cancelled = physicalBatch({ source: 'indexeddb', beforeSeq: 2, limit: 1, byteLimit: 100 });
  ...
  it('rejects a network page whose receipt belongs to another generation or channel', async () => {
    ...
    const batch = physicalBatch({ source: 'network', beforeSeq: 7, limit: 3, byteLimit: 100, generation: 8 });
    adapters.prepare(batch);
    await expect(adapters.execute(batch, { priority: 'foreground' })).rejects.toThrow('回执不匹配');
  ...
  it('accepts a network page only when its declared scan and row ranges are contiguous', async () => {
    ...
    const batch = physicalBatch({ source: 'network', beforeSeq: 7, limit: 3, byteLimit: 100, generation: 3 });
    adapters.prepare(batch);
    const execution = adapters.execute(batch, { priority: 'foreground' });
  ...
  it('validates cache pages against the same before cursor without inventing an EOF', async () => {
    ...
    const batch = physicalBatch({ source: 'indexeddb', beforeSeq: 7, limit: 3, byteLimit: 100, generation: 1 });
    adapters.prepare(batch);
  ...
  it('cancels an in-flight network page exactly once and ignores late rows', async () => {
    ...
    const batch = physicalBatch({ source: 'network', beforeSeq: 4, limit: 2, byteLimit: 100, generation: 1 });
    adapters.prepare(batch);
    const execution = adapters.execute(batch, { priority: 'foreground' });
  ...
  it('does not reopen a terminal source operation when a late page end arrives', async () => {
    ...
    const batch = physicalBatch({ source: 'indexeddb', beforeSeq: 4, limit: 2, byteLimit: 100, generation: 1 });
    adapters.prepare(batch);
