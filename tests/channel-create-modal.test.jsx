...
describe('ChannelCreateModal', () => {
  it('作为独立对话框提交真实创建命令并首先聚焦名称', async () => {
    ...
    render(<ChannelCreateModal channel={channel} channels={[]} roster={roster} state={emptyState()} onSubmit={onSubmit} onClose={() => {}} />);
  ...
  it('明确展示四步收敛，ready 后将新频道交给进入回调', async () => {
    ...
    const onSubmit = vi.fn().mockResolvedValue('request-1');
    const { rerender } = render(<ChannelCreateModal channel={channel} channels={[]} roster={roster} state={emptyState()} onSubmit={onSubmit} onClose={() => {}} onEnterChannel={onEnterChannel} />);
    await user.type(screen.getByLabelText('新频道名称'), 'research');
    await user.click(screen.getByRole('button', { name: '创建频道' }));

    rerender(<ChannelCreateModal channel={channel} channels={[{ id: 'new-id', qualified_name: 'c0.research', open: false, access: 'member_active' }]} roster={roster} state={{ turns: new Map([['request-1', completedTurn()]]) }} onSubmit={onSubmit} onClose={() => {}} onEnterChannel={onEnterChannel} />);
    ...
    rerender(<ChannelCreateModal channel={channel} channels={[readyChannel]} roster={roster} state={{ turns: new Map([['request-1', completedTurn()]]) }} onSubmit={onSubmit} onClose={() => {}} onEnterChannel={onEnterChannel} />);
  ...
  it('提交失败与账本失败都保留输入并允许重试', async () => {
    ...
    const { rerender } = render(<ChannelCreateModal channel={channel} channels={[]} roster={roster} state={emptyState()} onSubmit={onSubmit} onClose={() => {}} />);
    ...
    await user.click(screen.getByRole('button', { name: '创建频道' }));
    rerender(<ChannelCreateModal channel={channel} channels={[]} roster={roster} state={{ turns: new Map([['request-2', { terminal: { payload: { status: 'failed', reason: '名称已存在' } } }]]) }} onSubmit={onSubmit} onClose={() => {}} />);
  ...
  it('支持 Escape、遮罩关闭、焦点闭环和关闭后的焦点归还', async () => {
    ...
    const { rerender } = render(<><button ref={openerRef}>打开创建</button><ChannelCreateModal channel={channel} roster={roster} state={emptyState()} onSubmit={() => {}} onClose={onClose} returnFocusRef={openerRef} /></>);
