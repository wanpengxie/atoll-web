...
describe('提交生命周期竞态（useComposerSubmissionRuntime，与 tests/submission-outbox-current.test.jsx 同一 owner）', () => {
  ...
  it('does not record a receipt that arrives after the runtime really unmounts', async () => {
    let resolveReceipt;
    const receipt = new Promise((resolve) => { resolveReceipt = resolve; });
    const submit = vi.fn(() => receipt);
    const config = harness({ wireState: 'open', submit });
    const store = config.store;
    const putSubmission = vi.fn((...args) => store.putSubmission(...args));
    config.outboxFactory = () => ({ ...store, putSubmission, close: vi.fn() });
    ...
    await act(async () => { await sendPromise; await new Promise((resolve) => setTimeout(resolve, 20)); });
    expect(putSubmission.mock.calls.some(([, row]) => row.state === 'accepted')).toBe(false);
    store.close();
  });
