...
// 这条连接发出的每一条消息都盖着"从哪块屏来的"。
//
// 它盖在 wire 而不是各个调用点,理由跟发送者那条线一样:一个谁也绕不过去的出口
// 才盖得全。盖在调用点的话,漏掉的那条路径发出的消息上什么都没有,而读的人无从
// 分辨"这条没有来源"和"这条路径忘了盖"。
describe('每条消息都盖上从哪块屏发出的', () => {
  beforeEach(() => {
    FakeWebSocket.instances = [];
    vi.useFakeTimers();
  });

  const attachWith = (socket, session, label) => receipt(socket, socket.sent[0], {
    contract_version: 'v5', session, label, memberships: [], memberships_complete: true,
  });

  it('submit 的 body 上盖着 origin', async () => {
    const wire = createWire({ WebSocketImpl: FakeWebSocket });
    const socket = FakeWebSocket.instances[0];
    socket.open();
    attachWith(socket, 's-abc', 'Mac Chrome');
    await Promise.resolve();
    // 这条 submit 永远等不到回执(测试不发),close 时它会被拒——显式吞掉,
    // 否则未处理的 rejection 会污染整个文件的结果。
    wire.submit({ channel_id: 'c0', msg_type: 'agent.ask', kind: 'request', payload: { text: 'hi' } }).catch(() => {});
    const sent = socket.sent.find((item) => item.frame_type === 'submit');
    expect(sent.payload.payload).toEqual({
      text: 'hi',
      origin: { session: 's-abc', label: 'Mac Chrome' },
    });
    wire.close();
  });

  // 已经说了从哪来的就不覆盖:代转的消息,来源是原始那块屏,不是转发它的这条连接。
  it('已有的 origin 不被覆盖', async () => {
    const wire = createWire({ WebSocketImpl: FakeWebSocket });
    const socket = FakeWebSocket.instances[0];
    socket.open();
    attachWith(socket, 's-abc', 'Mac Chrome');
    ...
    const sent = socket.sent.find((item) => item.frame_type === 'submit');
    expect(sent.payload.payload.origin).toEqual({ session: 's-phone' });
    wire.close();
  });

  // 服务器没给名字就不编一个。空的 origin 比没有 origin 更糟:它看起来像个答案。
  it('服务器没给 session 就不盖', async () => {
    const wire = createWire({ WebSocketImpl: FakeWebSocket });
    const socket = FakeWebSocket.instances[0];
    socket.open();
    attachWith(socket, '', '');
    ...
    wire.submit({ channel_id: 'c0', msg_type: 'agent.ask', kind: 'request', payload: { text: 'hi' } }).catch(() => {});
    const sent = socket.sent.find((item) => item.frame_type === 'submit');
    expect(sent.payload.payload).toEqual({ text: 'hi' });
    wire.close();
  });
});
