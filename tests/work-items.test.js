...
describe('F4 WorkItem 索引', () => {
  it('统一审批、回合、正式任务、恢复和本设备自动动作，且按原生编号去重', () => {
    const state = fold([
      ...
      row(2, env('run-1', 'request', 'agent.ask', { text: '整理报告' })),
    ...
    const index = buildWorkItemIndex({
      ...
      timers: [{ timerId: 'timer-1', channelId: 'c1', durationMs: 1000, msgType: 'agent.ask', payload: { text: '周报提醒' }, createdAt: 1, dueAt: 1001, state: 'scheduled' }],
    });
  ...
  it('只接受 describe 明确声明 task.create 的 provider', () => {
    const providers = taskProviders(new Map([
      ...
      ['viewer', { actorId: 'viewer', describe: { types: new Map([['agent.ask', { allowedKinds: ['request'] }]]) } }],
