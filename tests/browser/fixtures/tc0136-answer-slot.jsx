...
const stageID = 'stage-text-1';
const terminalID = 'terminal-1';
const stageText = 'sealed answer target\n\n```mermaid\ngraph LR\n  A --> B\n```';
const rewrittenText = 'a genuinely rewritten terminal answer';
...
function processObservation() {
  ...
  return { seq: 2, envelope, process: envelope.payload.body.process };
}

function rowFor(terminalText = null, revision = 1) {
  const request = {
    id: requestID,
    kind: 'request',
    type: 'agent.ask',
  ...
  const turn = {
    ...
    request,
    requestSeq: 1,
    lastSeq: terminal ? 4 : 2,
    provisional: [processObservation()],
    terminal,
...
function AnswerSlotHarness({ model }) {
  const { renderRow } = useTimelineRowRenderer({
    state: { channelId: 'tc0136', narration: [] },
    names: new Map([
      ['human-1', { name: 'Me' }],
      ['agent-1', { name: 'Writer' }],
    ]),
  ...
  return renderRow(rowFor(model.terminalText, model.revision));
}

let setModel;
function App() {
  const [model, update] = useState({ terminalText: null, revision: 1 });
  setModel = update;
...
window.answerSlot = {
  ...
  rewrite() {
    flushSync(() => setModel({ terminalText: rewrittenText, revision: 2 }));
  },
  identity() {
