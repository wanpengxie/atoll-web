// 恢复自 master:tests/browser/f4-tasks.spec.js（RT 域）。
// 生产路径：App → WorkspaceApp → WorkspaceFeatures/TasksFeature/WorkspaceRightPanel，
// 走真实 Chromium + mock 服务端，不 mock 生产路径本身。
...
test('F4-001..003/005 Tasks 聚合审批且无 provider 时不伪造正式任务', async ({ page, request }) => {
  ...
  // 旧合同：详情面板必须能深链、刷新后恢复，并能回到原始动态。
  // 新结构的 panel 状态是纯 React useState（WorkspaceApp.jsx `const [panel, setPanel] = useState('')`），
  // 没有任何 URL 同步；如实断言，红了就是真回归。
  ...
  await page.getByRole('complementary', { name: '工作项详情' }).getByRole('button', { name: '返回来源' }).click();
...
test('F4-006 timer 只作为本设备 Automation 并在 320px 保持可达', async ({ page, request }) => {
  ...
  const tasks = page.getByRole('tabpanel', { name: '任务' });
  // 旧断言：安排的定时动作应该出现在任务列表里，可点开取消。保留原始断言——
  // 当前 composition 未把 automationRecords 接入 selectFeatureTaskFacts，
  // 因此只要服务端回执没有公开记录投影，这里必须如实标红。
