...
// Round-15 evidence stays on the current public search owner. The old
// activity contract supplied `operations`; the current public projection only
// accepts Replica/tasks/files/roster facts, so these cases deliberately keep
// the operation input at the public call boundary and record the result.
...
describe('A-D round 15 Activity / Operation blocked evidence', () => {
  it.fails('[AD-002] exposes terminal, WorkItem, and Operation as one locatable business fact', () => {
    ...
    // Current public owner: selectFeatureSearchIndex. Current result: the
    // operation projection is absent; this is a preserved owner-gap packet.
    expect(index.filter((entry) => entry.kind === 'operation')).toHaveLength(1);
  });

  it.fails('[AD-003] deduplicates Operation by channel/native id and retains the latest unsettled state', () => {
  ...
  it.fails('[AD-004] searches visible channels across operations with a public SourceRef', () => {
