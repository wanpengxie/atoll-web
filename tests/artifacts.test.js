...
describe('Artifact 账本索引', () => {
  it('只从合法附件建立频道隔离索引并保留 SourceRef', () => {
    ...
    apply(c1, row('c1', 4, { id: 'm1', kind: 'request', type: 'human.text', payload: { text: '报告', attachments: [{ resource_id: 'file:report', name: '报告.pdf', media_type: 'application/pdf', size: 42 }] } }));
    apply(c2, row('c2', 7, { id: 'm2', kind: 'request', type: 'human.text', payload: { attachments: [{ resource_id: 'file:report', name: '另一个报告.pdf', media_type: 'application/pdf' }] } }));
  ...
  it('不会从文件名、普通 JSON、ticket 或 registrar value 猜测产物', () => {
    ...
    apply(state, row('c0', 1, { id: 'm1', kind: 'request', type: 'human.text', payload: { text: '请查看 report-v2-final.pdf' } }));
    apply(state, row('c0', 2, { id: 'm2', kind: 'event', type: 'system.note', payload: { ticket: 'secret', address: 'daemon://hidden/path' } }));
    apply(state, row('c0', 4, { id: 'm4', kind: 'event', type: 'activity.tool.ended', payload: { attachments: [{ resource_id: 'event-file', name: 'not-terminal.pdf' }] } }));
  ...
  it('只接受显式版本关系并合并同资源引用', () => {
    const state = createChannelState('c0');
    apply(state, row('c0', 1, { id: 'v1', kind: 'request', type: 'human.text', payload: { attachments: [{ resource_id: 'r1', name: 'report-final.pdf', media_type: 'application/pdf' }] } }));
    apply(state, row('c0', 2, { id: 'v2', kind: 'request', type: 'human.text', payload: { attachments: [{ resource_id: 'r2', name: 'report-final-v2.pdf', media_type: 'application/pdf', version_of: 'r1' }] } }));
    apply(state, row('c0', 3, { id: 'again', kind: 'request', type: 'human.text', payload: { attachments: [{ resource_id: 'r1', name: 'renamed.pdf', media_type: 'application/pdf' }] } }));
  ...
  it('仅通过正式 artifact 对象或注册 adapter 接纳结构化产物', () => {
    const state = createChannelState('c0');
    apply(state, row('c0', 1, { id: 'q', kind: 'request', type: 'human.text', payload: { text: 'go' } }));
