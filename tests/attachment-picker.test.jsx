...
describe('Composer 附件来源', () => {
  it('本机文件先以当前频道 resource ticket 上传，再形成可发送附件', async () => {
    ...
    const attachment = await uploadChannelFile({ file, channel: { id: 'c0', qualified_name: 'c0' }, daemonId: 'local-device', onResource, fetchImpl });
    ...
    expect(fetchImpl).toHaveBeenCalledWith(expect.stringContaining('/files/daemon%3A%2F%2Flocal-device%2Fc0%2F'), expect.objectContaining({ method: 'PUT', body: file, credentials: 'include' }));
  ...
  it('daemon 文件在独立弹层中浏览，选择后返回结构化附件', async () => {
    ...
    render(<ChannelFilePickerModal channel={{ id: 'c0', qualified_name: 'c0' }} daemons={[{ id: 'local-device', name: '本机' }]} onResource={onResource} onChoose={onChoose} onClose={onClose} />);
