...
- WebSocket：`GET /ws`，帧信封版本 `v: 3`
- 只读观察：`GET /obs/space/*` 与 `GET /obs/channel/{id}/*`

连接打开后第一帧是且只能是 `attach { since, focus, history_protocol: 3 }`。历史回放和实时消息都来自下行 `feed`，历史页由相关的 `page_end` 收口；写消息、审批和频道管理糖衣分别使用 `submit`、`resolve` 等 v3 上行帧。每个频道的消息尾巴按行缓存在 IndexedDB，游标与已读位置仍是很小的 localStorage 数据。
...
测试覆盖 v3 帧构造/解析、信封状态代数、feed fold、游标单调性、带回执/错误/重连的 WebSocket 行为，以及真实启动本地 mock 后的登录、回放、submit、审批和 `since` 续传。

## 前端诊断

前端关键链路使用结构化日志，浏览器 Console 可按 `[atoll:` 过滤。连接、attach、历史调度、缓存、终端和 React 运行时异常都有独立事件名；日志不记录消息正文、协议 payload、凭据或 token。

当前标签页保留最近 500 条飞行记录，warning/error 同时写入 sessionStorage，刷新后仍可查看。在 DevTools Console 中：

```js
window.__ATOLL_DIAGNOSTICS__.snapshot()   // 结构化记录
copy(window.__ATOLL_DIAGNOSTICS__.exportText()) // 复制完整诊断 JSON
window.__ATOLL_DIAGNOSTICS__.clear()      // 清空本标签页诊断
```
