...
- Focused candidates: 30 files / 143 cases → 135 passed, 8 failed, plus one Outbox teardown DatabaseClosedError.
- Disposition: 115 GREEN, 5 RED, 1 PARTIAL, 187 OPEN, 20 GAP, 3 OBSOLETE. Action rows: 213 (OPEN + GAP + PARTIAL + RED).
- Product regressions without source changes: daemon secret projection; StructuredResult requestType/failure presentation; Outbox reconnect hot-loop; Outbox hydration resurrection; processing stop-control gate. Extra WorkspaceLayout pointer-focus is separately listed.

## Suite contracts

| ID | Baseline suite | Cases | Capability / invariant | Current public owner | Disposition |
|---|---|---:|---|---|---|
| S01 | tests/send-scroll-transaction.test.js | 10 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | current public owner boundary for send-scroll-transaction; successor still required | 0 GREEN / 0 RED / 10 OPEN / 0 GAP / 0 OBSOLETE |
| S02 | tests/server-boot.test.js | 3 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | ChannelFeedRuntime.setHistoryGrants + world Replica | 2 GREEN / 0 RED / 0 OPEN / 0 GAP / 1 OBSOLETE |
| S03 | tests/space-administration.test.js | 8 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | useWireConnection accessRef.directory + governance ports | 0 GREEN / 1 RED / 0 OPEN / 7 GAP / 0 OBSOLETE |
| S04 | tests/structured-result-ui.test.jsx | 2 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | TimelineRowRenderer StructuredResult | 2 GREEN / 0 RED / 0 OPEN / 0 GAP / 0 OBSOLETE |
| S05 | tests/structured-result.test.js | 4 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | TimelineRowRenderer useTimelineRowRenderer | 2 GREEN / 2 RED / 0 OPEN / 0 GAP / 0 OBSOLETE |
| S06 | tests/submission-outbox.test.jsx | 16 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | useComposerSubmissionRuntime + outbox-store | 9 GREEN / 2 RED / 3 OPEN / 1 GAP / 0 OBSOLETE |
| S07 | tests/submissions.test.js | 4 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | useComposerSubmissionRuntime pending/reconcileFeed | 4 GREEN / 0 RED / 0 OPEN / 0 GAP / 0 OBSOLETE |
| S08 | tests/surface-shell-visual-viewport.test.jsx | 1 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | current public owner boundary for surface-shell-visual-viewport; successor still required | 1 GREEN / 0 RED / 0 OPEN / 0 GAP / 0 OBSOLETE |
| S09 | tests/sync-data-fuzz.test.js | 10 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | channel-replica + sync-session | 10 GREEN / 0 RED / 0 OPEN / 0 GAP / 0 OBSOLETE |
| S10 | tests/system-events.test.js | 3 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | current public owner boundary for system-events; successor still required | 0 GREEN / 0 RED / 3 OPEN / 0 GAP / 0 OBSOLETE |
| S11 | tests/task-controls.test.js | 9 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | WaitingLayer public component | 9 GREEN / 0 RED / 0 OPEN / 0 GAP / 0 OBSOLETE |
| S12 | tests/tasks-f4.test.jsx | 4 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | TasksFeature public port | 4 GREEN / 0 RED / 0 OPEN / 0 GAP / 0 OBSOLETE |
| S13 | tests/terminal-view.test.jsx | 14 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | TerminalFeature + terminal-session | 0 GREEN / 0 RED / 14 OPEN / 0 GAP / 0 OBSOLETE |
| S14 | tests/test-integrity-contract.test.js | 6 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | runner/test setup contract | 6 GREEN / 0 RED / 0 OPEN / 0 GAP / 0 OBSOLETE |
| S15 | tests/timeline-agent-activity.test.jsx | 2 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | current public owner boundary for timeline-agent-activity; successor still required | 1 GREEN / 0 RED / 1 OPEN / 0 GAP / 0 OBSOLETE |
| S16 | tests/timeline-channel-switch.test.jsx | 12 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | current public owner boundary for timeline-channel-switch; successor still required | 0 GREEN / 0 RED / 12 OPEN / 0 GAP / 0 OBSOLETE |
| S17 | tests/timeline-filter-presentation.test.jsx | 20 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | current public owner boundary for timeline-filter-presentation; successor still required | 2 GREEN / 0 RED / 18 OPEN / 0 GAP / 0 OBSOLETE |
| S18 | tests/timeline-presentation-identity.test.js | 20 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | conversation-presentation evaluate/commitCandidate | 14 GREEN / 0 RED / 6 OPEN / 0 GAP / 0 OBSOLETE |
| S19 | tests/timeline-reading-integration.test.jsx | 71 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | useReadingSession + projection + VendorListExecutor | 0 GREEN / 0 RED / 71 OPEN / 0 GAP / 0 OBSOLETE |
| S20 | tests/timeline-render-authority.test.jsx | 1 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | current public owner boundary for timeline-render-authority; successor still required | 0 GREEN / 0 RED / 1 OPEN / 0 GAP / 0 OBSOLETE |
...
| S22 | tests/timeline-scope-incremental.test.js | 3 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | current public owner boundary for timeline-scope-incremental; successor still required | 0 GREEN / 0 RED / 3 OPEN / 0 GAP / 0 OBSOLETE |
| S23 | tests/timeline-scope.test.js | 16 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | current public owner boundary for timeline-scope; successor still required | 0 GREEN / 0 RED / 16 OPEN / 0 GAP / 0 OBSOLETE |
| S24 | tests/timers.test.js | 2 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | current public owner boundary for timers; successor still required | 1 GREEN / 0 RED / 1 OPEN / 0 GAP / 0 OBSOLETE |
| S25 | tests/turn-presentation.test.js | 1 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | current public owner boundary for turn-presentation; successor still required | 0 GREEN / 0 RED / 1 OPEN / 0 GAP / 0 OBSOLETE |
| S26 | tests/ui-activity-overlay.test.jsx | 6 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | no current ui.* receipt consumer | 0 GREEN / 0 RED / 0 OPEN / 6 GAP / 0 OBSOLETE |
| S27 | tests/ui-primitives.test.jsx | 7 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | current public owner boundary for ui-primitives; successor still required | 6 GREEN / 0 RED / 1 OPEN / 0 GAP / 0 OBSOLETE |
| S28 | tests/ui-words-hook.test.jsx | 6 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | no current ui.* command port | 0 GREEN / 0 RED / 0 OPEN / 6 GAP / 0 OBSOLETE |
| S29 | tests/version-incompatible.test.jsx | 1 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | WorkspaceApp incompatible port + UI | 1 GREEN / 0 RED / 0 OPEN / 0 GAP / 0 OBSOLETE |
| S30 | tests/view-session.test.js | 8 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | view-session public store | 6 GREEN / 0 RED / 0 OPEN / 0 GAP / 2 OBSOLETE |
| S31 | tests/visual-interaction-contract.test.jsx | 6 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | VendorListExecutor + reading-dom-command-executor + SurfaceShell | 6 GREEN / 0 RED / 0 OPEN / 0 GAP / 0 OBSOLETE |
| S32 | tests/waiting-closure-entry.test.jsx | 6 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | current public owner boundary for waiting-closure-entry; successor still required | 0 GREEN / 0 RED / 6 OPEN / 0 GAP / 0 OBSOLETE |
| S33 | tests/waiting-layout.test.jsx | 3 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | ConversationSurface + WaitingLayer + Composer layout | 3 GREEN / 0 RED / 0 OPEN / 0 GAP / 0 OBSOLETE |
| S34 | tests/waiting-presentation.test.js | 6 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | current public owner boundary for waiting-presentation; successor still required | 0 GREEN / 0 RED / 6 OPEN / 0 GAP / 0 OBSOLETE |
| S35 | tests/waiting-terminal-finality.test.js | 11 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | current public owner boundary for waiting-terminal-finality; successor still required | 0 GREEN / 0 RED / 11 OPEN / 0 GAP / 0 OBSOLETE |
| S36 | tests/wire.test.js | 18 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | createWire public client | 18 GREEN / 0 RED / 0 OPEN / 0 GAP / 0 OBSOLETE |
| S37 | tests/work-items.test.js | 4 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | feature-tasks + task feature port | 4 GREEN / 0 RED / 0 OPEN / 0 GAP / 0 OBSOLETE |
| S38 | tests/workspace-bootstrap-cache.test.js | 1 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | workspace-bootstrap-cache public store | 1 GREEN / 0 RED / 0 OPEN / 0 GAP / 0 OBSOLETE |
| S39 | tests/workspace-route.test.js | 3 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | useChannelNavigation route owner | 2 GREEN / 0 RED / 1 OPEN / 0 GAP / 0 OBSOLETE |

## Complete case ledger

GREEN=current public-owner proof; RED=product regression; PARTIAL=related path only; OPEN=successor not built; GAP=no current owner; OBSOLETE=retired oracle explicitly accounted.
...
| SZ-011 | tests/server-boot.test.js — records the first boot without invalidating state created by this login | S02 | **OBSOLETE** — retired storage/implementation shape; current fact documented |
...
| SZ-025 | tests/structured-result.test.js — never renders successful non-text results as an empty answer | S05 | **RED** — current public-owner repro fails baseline assertion; hand to owner, no source change here |
| SZ-026 | tests/structured-result.test.js — keeps failure facts and redacts sensitive fields recursively | S05 | **RED** — current public-owner repro fails baseline assertion; hand to owner, no source change here |
...
| SZ-035 | tests/submission-outbox.test.jsx — does not downgrade an in-flight submission when access changes before its accepted receipt | S06 | **RED** — current public-owner repro fails baseline assertion; hand to owner, no source change here |
...
| SZ-039 | tests/submission-outbox.test.jsx — treats feed-before-receipt as landed without retransmitting or emitting another send phase | S06 | **RED** — current public-owner repro fails baseline assertion; hand to owner, no source change here |
...
| SZ-078 | tests/terminal-view.test.jsx — 连接 ready 前的输入不会静默丢失 | S13 | **OPEN** — retained; build public-owner successor or hand off regression |
...
| SZ-272 | tests/view-session.test.js — migrates viewport unseen state from finite records and discards legacy derived surplus | S30 | **OBSOLETE** — retired storage/implementation shape; current fact documented |
| SZ-273 | tests/view-session.test.js — drops count-only and key-only viewport unseen state at the storage boundary | S30 | **OBSOLETE** — retired storage/implementation shape; current fact documented |
...
| Packet | Repro | First divergence | Capability/invariant |
|---|---|---|---|
| R-SZ-001 | space-administration public accessRef.directory receives daemon key=secret. | loadSpaceDirectory/projectDirectoryRows returns key. | directory projection must not expose declaration secrets. |
| R-SZ-002 | structured-result-restore renders system.channel.list, actor.describe, type_unsupported through public TimelineRowRenderer. | StructuredResult lacks requestType/failure labels. | preserve protocol reason/type and redaction in user language. |
| R-SZ-003 | submission-outbox uncertain id, open/reconnecting/open twice; submit count 3 not 2. | reconnect effect retransmits processed id. | one durable intent/lease per reconnect; no hot loop. |
| R-SZ-004 | submission-outbox feed before principal hydration; row remains in IndexedDB after open rerender. | hydration restores without landed removal. | canonical feed ack cannot resurrect. |
| R-SZ-005 | task-controls processing turn without advertised agent.interrupt still shows 停止. | TurnCard bypasses access/capability gate. | controls require current writable authority. |
| R-SZ-X01 extra | workspace-layout-channel-focus click c1 then rerender; heading not focused. | click path selects but does not schedule focus. | committed selection transfers focus without scroll. |
