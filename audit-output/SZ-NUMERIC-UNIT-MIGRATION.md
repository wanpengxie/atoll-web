...
- Disposition: 164 GREEN, 0 RED, 1 PARTIAL, 142 OPEN, 20 GAP, 3 BLOCKED. Action rows: 166 (OPEN + GAP + PARTIAL + RED + BLOCKED pending ruling).
- The daemon secret projection is now green through the existing `useWireSession` display allow-list and its public `accessRef.directory()` test. The separately listed WorkspaceLayout pointer-focus packet remains outside this round. R-SZ-002A/B/C and R-SZ-005 are resolved in the public renderer; Outbox race evidence is green in the current Composer owner and remains handoff-only for any further product work, with no duplicate packet here.
...
| S03 | tests/space-administration.test.js | 8 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | useWireConnection accessRef.directory + governance ports | 1 GREEN / 0 RED / 0 OPEN / 7 GAP / 0 BLOCKED |
...
| S06 | tests/submission-outbox.test.jsx | 16 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | useComposerSubmissionRuntime + outbox-store | 11 GREEN / 0 RED / 3 OPEN / 1 GAP / 0 BLOCKED |
...
| S15 | tests/timeline-agent-activity.test.jsx | 2 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | current public owner boundary for timeline-agent-activity; successor still required | 2 GREEN / 0 RED / 0 OPEN / 0 GAP / 0 BLOCKED |
| S16 | tests/timeline-channel-switch.test.jsx | 12 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | current public owner boundary for timeline-channel-switch; successor still required | 3 GREEN / 0 RED / 9 OPEN / 0 GAP / 0 BLOCKED |
| S17 | tests/timeline-filter-presentation.test.jsx | 20 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | current public owner boundary for timeline-filter-presentation; successor still required | 4 GREEN / 0 RED / 16 OPEN / 0 GAP / 0 BLOCKED |
| S18 | tests/timeline-presentation-identity.test.js | 20 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | conversation-presentation evaluate/commitCandidate | 16 GREEN / 0 RED / 4 OPEN / 0 GAP / 0 BLOCKED |
...
| S22 | tests/timeline-scope-incremental.test.js | 3 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | current public owner boundary for timeline-scope-incremental; successor still required | 2 GREEN / 0 RED / 1 OPEN / 0 GAP / 0 BLOCKED |
| S23 | tests/timeline-scope.test.js | 16 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | `selectTimelineItems` in `src/model/conversation-presentation.js`, exercised through `createChannelReplicaStore` | 15 GREEN / 0 RED / 1 OPEN / 0 GAP / 0 BLOCKED |
...
| S27 | tests/ui-primitives.test.jsx | 7 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | current public owner boundary for ui-primitives; successor still required | 7 GREEN / 0 RED / 0 OPEN / 0 GAP / 0 BLOCKED |
...
| S32 | tests/waiting-closure-entry.test.jsx | 6 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | createChannelReplicaStore + feature-tasks Waiting projection | 1 GREEN / 0 RED / 5 OPEN / 0 GAP / 0 BLOCKED |
...
| S34 | tests/waiting-presentation.test.js | 6 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | `useWaitingEditingController` in `src/ui/timeline/useWaitingEditingController.jsx` | 6 GREEN / 0 RED / 0 OPEN / 0 GAP / 0 BLOCKED |
...
| SZ-018 | tests/space-administration.test.js — projects daemon observations without secret fields | S03 | **GREEN** — `useWireConnection` exposes the allow-listed daemon display projection through `accessRef.directory().devices`; the direct public-owner test asserts display fields, no `key`, and no serialized secret. |
...
| SZ-035 | tests/submission-outbox.test.jsx — does not downgrade an in-flight submission when access changes before its accepted receipt | S06 | **GREEN / HANDOFF OWNER** — `useComposerSubmissionRuntime` keeps the in-flight record transmitting through an access change and accepts the receipt; direct owner test passes. Any further Outbox product work remains with `composer_owner`; no duplicate packet here. |
...
| SZ-039 | tests/submission-outbox.test.jsx — treats feed-before-receipt as landed without retransmitting or emitting another send phase | S06 | **GREEN / HANDOFF OWNER** — `useComposerSubmissionRuntime.reconcileFeed` clears the in-flight record before the delayed receipt and the direct owner test observes one submit with no duplicate phase; composer remains the sole Outbox owner. |
...
| SZ-096 | tests/timeline-agent-activity.test.jsx — balances consumers without treating A→B→A replacement as visible-arrival acknowledgement | S15 | **GREEN** — `tests/live-timeline-arrivals.test.js` exercises the public `channel-replica.arrivalReceipts` owner through A→B→A detach/reattach and proves only an explicit receipt clears the pending arrival. |
| SZ-097 | tests/timeline-channel-switch.test.jsx — 换频道就换一棵消息树，恒不复用同一个 DOM 容器 | S16 | **GREEN** — `src/ui/conversation/channel-switch-identity.test.jsx` proves the public conversation surface remounts a distinct reading tree on channel change. |
| SZ-098 | tests/timeline-channel-switch.test.jsx — 反复切频道后，消息区恒只有一棵 | S16 | **GREEN** — `src/ui/conversation/channel-switch-identity.test.jsx` repeats channel switches and asserts one mounted reading tree. |
| SZ-099 | tests/timeline-channel-switch.test.jsx — 消息区这一层恒不出现重复 key 警告 | S16 | **GREEN** — `src/ui/conversation/channel-switch-identity.test.jsx` spies on console errors across repeated switches and observes no duplicate-key warning. |
...
| SZ-122 | tests/timeline-filter-presentation.test.jsx — 成员过滤按钮收窄呈现条目 | S17 | **GREEN** — `src/ui/conversation/scope-and-actor-filter.test.jsx` drives the real ConversationSurface member-filter chip and observes the filtered presentation state. |
| SZ-123 | tests/timeline-filter-presentation.test.jsx — 旧incarnation的持久筛选始终可见可移除，不装作未过滤空态 | S17 | **GREEN** — `src/ui/conversation/scope-and-actor-filter.test.jsx` keeps a stale actor chip visible and removable rather than presenting an unfiltered empty state. |
...
| SZ-145 | tests/timeline-presentation-identity.test.js — keeps the same candidate across prepend and excludes control-only tail rows | S18 | **GREEN** — `tests/timeline-presentation-identity.test.js` proves the current-entry candidate remains the same through a prepend and ignores a control-only tail. |
...
| SZ-147 | tests/timeline-presentation-identity.test.js — publishes a projection candidate exactly once after commit | S18 | **GREEN** — `tests/timeline-presentation-identity.test.js` evaluates a candidate, commits it once, and rejects the same receipt on a second commit. |
...
| SZ-224 | tests/timeline-scope-incremental.test.js — 逐帧生长时,每一帧都与全量版相等 | S22 | **GREEN** — `src/model/conversation-presentation-scope.test.js` checks every appended frame through the public `selectTimelineItems` projection, including retroactive correlation backfill. |
...
| SZ-226 | tests/timeline-scope-incremental.test.js — 换了 selfId 就整张重建,恒不在别人的基线上继续加 | S22 | **GREEN** — `src/model/conversation-presentation-scope.test.js` switches self IDs on the same Replica and proves neither scope carries the other person's exchange. |
| SZ-227 | tests/timeline-scope.test.js — reaches the whole exchange from the one message I sent | S23 | **GREEN** — `src/model/conversation-presentation-mine-rules.test.js` proves the public `selectTimelineItems` owner retains the complete related exchange from a direct human seed. |
| SZ-228 | tests/timeline-scope.test.js — leaves an exchange that is not mine out | S23 | **GREEN** — `src/model/conversation-presentation-mine-rules.test.js` proves an unrelated exchange is excluded from the Mine projection. |
| SZ-229 | tests/timeline-scope.test.js — keeps a whole conversation when any part of it is mine, and drops the ones that are not | S23 | **GREEN** — `src/model/conversation-presentation-mine-rules.test.js` proves the mine-related turn keeps its terminal/thread while the other conversation is omitted. |
| SZ-230 | tests/timeline-scope.test.js — changes nothing under the全部 scope, and nothing when there is no self to scope by | S23 | **GREEN** — `src/model/conversation-presentation-mine-rules.test.js` proves All remains complete and missing self identity does not silently empty the projection. |
...
| SZ-232 | tests/timeline-scope.test.js — keeps the exact edit replacement candidate out until reciprocal handoff | S23 | **GREEN** — `src/model/conversation-presentation-mine-rules.test.js` proves the replacement is withheld only while the exact edit handoff is pending. |
| SZ-233 | tests/timeline-scope.test.js — 终端命令在「@我」下不出现——我在终端里已经全程看着了 | S23 | **GREEN** — `src/model/conversation-presentation-mine-rules.test.js` proves `terminal.command` is excluded from Mine while remaining a canonical ledger fact. |
| SZ-234 | tests/timeline-scope.test.js — 终端会话开关在「@我」下同样不出现 | S23 | **GREEN** — `src/model/conversation-presentation-mine-rules.test.js` proves both terminal session transitions stay out of Mine. |
| SZ-235 | tests/timeline-scope.test.js — 但在「全部」下照常可见——账本恒是完整的 | S23 | **GREEN** — `src/model/conversation-presentation-mine-rules.test.js` proves the terminal command remains visible under All. |
| SZ-236 | tests/timeline-scope.test.js — 恒不因为它而把整段对话捞进来 | S23 | **GREEN** — `src/model/conversation-presentation-mine-rules.test.js` proves a self terminal record cannot seed an unrelated correlation into Mine. |
| SZ-237 | tests/timeline-scope.test.js — 真实 timer fire、唤醒请求、进展和子请求都在 | S23 | **GREEN** — `src/model/conversation-presentation-mine-rules.test.js` proves canonical timer fire plus wake/progress/child facts remain in the Mine closure. |
| SZ-238 | tests/timeline-scope.test.js — peer 分页里的普通 self-audience 请求仍不算 timer | S23 | **GREEN** — `src/model/conversation-presentation-mine-rules.test.js` proves an ordinary peer self-audience request is not admitted as a timer conversation. |
| SZ-239 | tests/timeline-scope.test.js — 只有 timer 前缀不够，普通 self event 不获得归属 | S23 | **GREEN** — `src/model/conversation-presentation-mine-rules.test.js` proves a timer-looking id with a mismatched correlation is not a timer seed. |
| SZ-240 | tests/timeline-scope.test.js — 一旦同 correlation 有 human 边，自委托仍由二层闭包保留 | S23 | **GREEN** — `src/model/conversation-presentation-mine-rules.test.js` proves a delegated child is retained under the human-root conversation via parent/correlation closure. |
| SZ-241 | tests/timeline-scope.test.js — 普通 self-audience 请求仍在「全部」账本可见 | S23 | **GREEN** — `src/model/conversation-presentation-mine-rules.test.js` proves the ordinary self-audience request remains visible under All. |
| SZ-242 | tests/timeline-scope.test.js — 人给自己发的不算委托——那条线走的是"我发/我收"，不受这条规则影响 | S23 | **GREEN** — `src/model/conversation-presentation-mine-rules.test.js` proves a human self-send remains visible as the ordinary self conversation and is not admitted as delegated timer work. |
...
| SZ-258 | tests/ui-primitives.test.jsx — 进入时聚焦取消，Escape 取消并在卸载后返回焦点 | S27 | **GREEN** — `tests/ui-primitives.test.jsx` drives the real `InlineConfirmation` focus, Escape cancel, and unmount focus return contract. |
...
| SZ-281 | tests/waiting-closure-entry.test.jsx — releases the newer terminal visible-gap page before the older request page | S32 | **GREEN** — `tests/waiting-replay-owner.test.js` drives the current Replica owner with a terminal-first page, trim, then the older request/queued page; the exact terminal closure remains authoritative and `selectFeatureWaitingFacts` exposes no resurrected Waiting action. |
...
| SZ-289 | tests/waiting-presentation.test.js — assigns a durable local agent request to Waiting before ledger position arrives | S34 | **GREEN** — `src/ui/timeline/waiting-presentation.test.jsx` proves the public Waiting controller admits only the accepted local agent request before ledger position exists. |
| SZ-290 | tests/waiting-presentation.test.js — keeps newly accepted local requests after canonical queued positions | S34 | **GREEN** — `src/ui/timeline/waiting-presentation.test.jsx` proves canonical queued order is retained and the newly accepted local request remains in Waiting. |
| SZ-291 | tests/waiting-presentation.test.js — keeps a canonical queued fact visible while target authority is resolved elsewhere | S34 | **GREEN** — `src/ui/timeline/waiting-presentation.test.jsx` proves the canonical queued fact stays visible independent of target authority resolution. |
| SZ-292 | tests/waiting-presentation.test.js — keeps one stable id through local, landed-open, and canonical queued commits | S34 | **GREEN** — `src/ui/timeline/waiting-presentation.test.jsx` proves local and canonical commits collapse to one request id, without a duplicate local echo. |
| SZ-293 | tests/waiting-presentation.test.js — never guesses that an unrelated open request is queued | S34 | **GREEN** — `useWaitingEditingController.latestStage` now admits canonical Waiting only from explicit `received/queued/deferred` provisional facts; `src/ui/timeline/waiting-presentation.test.jsx` proves no-provisional exclusion, explicit queued admission, and terminal exclusion. |
| SZ-294 | tests/waiting-presentation.test.js — releases local continuity when the canonical request starts running | S34 | **GREEN** — `src/ui/timeline/waiting-presentation.test.jsx` proves a canonical processing status removes the local Waiting continuity. |
...
|---|---|---|---|---|
| R-SZ-001 | Daemon directory projection never exposes declaration secrets. | `tests/space-administration.test.js`: public `accessRef.directory().devices` receives a fixture declaration with `key: secret`, while the public row exposes only display fields and serializes without the secret. | `useWireConnection` accessRef.directory + governance ports | GREEN; existing daemon allow-list plus direct owner guard closes SZ-018 |
...
| R-SZ-007 | Cached Waiting facts remain readable, but stale control-tail or roster authority must not expose action buttons; controls return only with both current facts. | `tests/feature-waiting-controls.test.jsx`: keeps a cached Waiting row visible while hiding all controls for missing/stale authority, then restores both controls only for current tail and roster authority. | `TasksFeature` WaitingRow + feature waiting authority facts | GREEN; minimal presentation-owner gate and direct public positive/negative tests passed |
...
| H-SZ-OUTBOX | Outbox reconnect hot-loop and feed-before-hydration resurrection remain single-owner handoff scope; no duplicate product packet here. | Current `tests/submission-outbox.test.jsx` direct owner cases for SZ-035 and SZ-039 pass: access change does not downgrade an in-flight receipt, and feed-before-receipt clears once with one submit. | `composer_owner`: `useComposerSubmissionRuntime` + `outbox-store` | GREEN EVIDENCE / HANDOFF OWNER; composer_owner remains sole owner for any further Outbox work |
