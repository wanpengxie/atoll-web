...
- Disposition: 115 GREEN, 5 RED, 1 PARTIAL, 187 OPEN, 20 GAP, 3 BLOCKED. Action rows: 216 (OPEN + GAP + PARTIAL + RED + BLOCKED pending ruling).
- Product regressions without source changes: daemon secret projection; three mutually exclusive StructuredResult presentation packets (two request-type titles and one failure label); processing stop-control permission; and the separately listed WorkspaceLayout pointer-focus packet. Outbox races are handoff-only to `composer_owner` and are not duplicated here.
...
| S05 | tests/structured-result.test.js | 4 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | TimelineRowRenderer useTimelineRowRenderer | 2 GREEN / 2 RED / 0 OPEN / 0 GAP / 0 BLOCKED |
...
| S13 | tests/terminal-view.test.jsx | 14 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | TerminalFeature + terminal-session | 0 GREEN / 0 RED / 14 OPEN / 0 GAP / 0 BLOCKED |
...
| SZ-025 | tests/structured-result.test.js — never renders successful non-text results as an empty answer | S05 | **RED** — baseline capability has two disjoint failing protocol branches, R-SZ-002A (`system.channel.list`) and R-SZ-002B (`actor.describe`); hand each to the public owner, no source change here |
| SZ-026 | tests/structured-result.test.js — keeps failure facts and redacts sensitive fields recursively | S05 | **RED** — R-SZ-002C (`type_unsupported` failure wording) fails through the public render path; hand to owner, no source change here |
...
| R-SZ-002A | StructuredResult uses the protocol request type as the title for `system.channel.list`. | `tests/structured-result-restore.test.jsx`: completed `system.channel.list` result must show `system.channel.list`, not generic `结构化结果`. | `useTimelineRowRenderer` → public `TimelineRowRenderer` structured-result branch | RED; baseline SZ-025 branch |
| R-SZ-002B | StructuredResult presents `actor.describe` with the actor capability title `codex 的能力`. | `tests/structured-result-restore.test.jsx`: completed `actor.describe` result must show `codex 的能力`, not generic `结构化结果`. | `useTimelineRowRenderer` → public `TimelineRowRenderer` structured-result branch | RED; baseline SZ-025 branch |
| R-SZ-002C | StructuredResult maps `type_unsupported` to the user-facing failure wording `接收方不支持这个操作`. | `tests/structured-result-restore.test.jsx`: failed result with `error_code: type_unsupported` must show that label while retaining detail/redaction behavior. | `useTimelineRowRenderer` → public `TimelineRowRenderer` failure branch | RED; baseline SZ-026 |
