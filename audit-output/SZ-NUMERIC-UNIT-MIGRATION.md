...
- Disposition: 147 GREEN, 4 RED, 1 PARTIAL, 156 OPEN, 20 GAP, 3 BLOCKED. Action rows: 184 (OPEN + GAP + PARTIAL + RED + BLOCKED pending ruling).
...
| S34 | tests/waiting-presentation.test.js | 6 | preserve every baseline user-visible capability/invariant named by its exact titles; implementation details are not an owner | `useWaitingEditingController` in `src/ui/timeline/useWaitingEditingController.jsx` | 5 GREEN / 1 RED / 0 OPEN / 0 GAP / 0 BLOCKED |
...
| SZ-293 | tests/waiting-presentation.test.js — never guesses that an unrelated open request is queued | S34 | **RED / HANDOFF ONLY** — `src/ui/timeline/waiting-presentation.test.jsx` reproduces the current public owner showing a canonical `agent.ask` with no provisional queued signal; baseline requires conservative non-guessing. Product ruling required; no source change here. |
