...
| 项目 | 已证事实 | 当前动作 |
|---|---|---|
| 吸底时点击状态闪屏 | layout-choice/fold/details/edit 被错误归类为 Reading navigation，导致 following 被冷 browsing 容器替换；多种 live-sibling bridge 均被 actual paint 否决 | Presentation/content interaction 保持原 mode/adapter；只有 native reading input 取得 browsing，不逐按钮补 transition |
