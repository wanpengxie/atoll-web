...
本文是 UI 组件层重构轨道的历史权威文档，包含目标设计、施工顺序、验收证据和完成状态。后续产品对象、布局与视觉以 [FRONTEND-PRODUCT-IMPROVEMENT-MASTER-PLAN.md](FRONTEND-PRODUCT-IMPROVEMENT-MASTER-PLAN.md) 和 D1/D2 为准。
...
- 左栏频道导航、中栏账本、右栏详情/管理区的表面分层；F6 统一为 Atoll 暖色 Light-only 方案，不保留历史“深色侧栏”冻结要求；
